...
- **Real-Time Manager**: new sign-in flow.

**Edge Orchestrator** 

- **Drift configuration management**: Edge Orchestrator now s grants that every file provisioned on Edge Nodes are the same defined in the Edge Service, restoring if any file is changed manually. 
- **Agent `--join-group` option**: this agent is able to register or bind itself into a node group on first start. 
- **Agent `--set-name` option**: this agent is able to set the node name on first start (default to hostname).
