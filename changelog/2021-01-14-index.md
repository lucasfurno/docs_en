...
* Bugfix to allow edit/add Data Streamings with Google BigQuery endpoint type.
* API V3 Metrics - It's now possible to change the timestamp format. The documentation can be found [here](https://www.azion.com/en/documentation/products/api/v3/real-time-metrics/).
* Data Streaming - Users can now define the separator that will be used in logs, for HTTP/HTTPS POST endpoint types. The documentation can be found [here](https://www.azion.com/en/documentation/products/data-streaming/).
* Real Time Events - Our filter was changed and now the queries must be performed with the same syntax used in SQL. The new documentation can be found [here](https://www.azion.com/en/documentation/products/real-time-events/).
* Added functionality for multiport access to Edge Application.
* Bugfix in Edge Firewall API.
* Bugfix in the Cache Settings list.
* Security improvement for creating rules in Rules Engine.
* Added support for using HTTP methods (POST, PUT, PATCH and DELETE) in Edge Functions JavaScript.
* New Sign-Up for mobile devices

**Abr/2021**

* Bugfix when the user cloned an Edge Application with Json Args.
* Data Streaming - ElasticSearch connector - The new connector is now available in Data Streaming for codeless integration. 
* Data Streaming - BigQuery connector - The new connector is now available in Data Streaming for codeless integration.
* Data Streaming - Data Source Edge Functions - We made these logs available in the Data Streaming product. 
* Bugfix when selecting a setting in Real-Time Metrics. 
* Bugfix searching for LastHour in the Real-Time Metrics API V3. 
* Compute Time metric of the Edge Function product presented on Billing UI for the customer.


**Mar/2021**

- New [API for Edge Application - Rules Engine](https://www.azion.com/en/documentation/products/api/v3/edge-firewall/rules-engine/);
- New [API for Edge Application - Edge Functions Instance](https://www.azion.com/en/documentation/products/api/v3/edge-applications/edge-functions-instances/);
- New [API for Edge Firewall - Main Settings](https://www.azion.com/pt-br/documentacao/produtos/api/v3/edge-firewall/);
- New [API for Edge Firewall - Rules Engine](https://www.azion.com/pt-br/documentacao/produtos/api/v3/edge-firewall/rules-engine/);
- Bugfix when occur cache on Set-Cookie;
- Launch [Edge Function's](https://www.azion.com/en/blog/why-azion-chose-v8/) General Availability;
- Bugfix for preventing CDN loops;
