...
* **Host URL:** it refers to the URL of the Host S3. You can connect with any provider that works with the S3 protocol.

  For example: *http://myownhost.s3.us-east-1.myprovider.com*

* **Bucket Name:** it refers to the name of the Bucket that the object will be sent to.
...
* **Region:**  it refers to the region in which the Bucket is hosted. 

  For example:  *us-east-1*.

* **Access Key:** it refers to the public key to access the Bucket, which is given by your provider.

  For example: *ORIA5ZEH9MW4NL5OITY4*

* **Secret Key:** it refers to the secret key to access the Bucket, which is given by your provider.

  For example: *+PLjkUWJyOLth3anuWXcLLVrMLeiiiThIokaPEiw*

* **Object Key Prefix:**  it refers to a prefix that you can add to the files that will be sent. 

  For example: if you use *waf_logs*, all the sent objects will be saved with *waf_logs__*.

* **Content Type:** it refers to the format in which the object will be created in the Bucket. 
