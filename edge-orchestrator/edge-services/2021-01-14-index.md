...
You can configure the  install, uninstall, and reload triggers and define the dependencies between the resources required to run your service on your Edge network.
...
In order to be able to orchestrate services on your device, you need to configure all the **resources** required to install, uninstall and reload your services.
...
All resources are executed using the *Path* referred to in the registry. The *Path* field refers to the absolute path of the directory. Relative paths will not be accepted and are not supported.
