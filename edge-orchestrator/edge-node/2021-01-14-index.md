...
> **Note**: The commands executed in the edge-orchestrator agent, must be executed with root user privileges when the Operating System used is Linux . 

The root user is the one who has unrestricted access to all components of the Linux system, whether files or processes. If you do not have these privileges you may experience installation failures when attempting to manage some operating system requirements that demand such privileges.
