...
if(set.mode=='static')
{tipuesearch_in=$.extend({},set.content_tipuesearch);}
...
if(getURLP('q'))
{$('#searchHeaderInput').val(getURLP('q'));getTipueSearch(0,true);}
$(this).keyup(function(event)
{if($('#searchHeaderInput').val().length > 4)
{getTipueSearch(0,true);}});function getTipueSearch(start,replace)
{var out='';var show_replace=false;var show_stop=false;var standard=true;var c=0;found=[];var d_o=$('#searchHeaderInput').val();var d=d_o.toLowerCase();d=$.trim(d);if((d.match("^\"")&&d.match("\"$"))||(d.match("^'")&&d.match("'$")))
{standard=false;}
...
{c_c=c.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",");out+='<div id="tipue_search_results_count">'+tipuesearch_string_16+' '+c_c+' '+tipuesearch_string_5;}
if(set.showTime)
...
out+='</ul></div></nav>';out=out+'<a class="search-show-more" href="#">'+tipuesearch_string_17+'</a>';}}
else
...
{$('#searchHeaderInput').val($(this).attr('id'));getTipueSearch(0,true);});$('.tipue_search_foot_box').click(function()
