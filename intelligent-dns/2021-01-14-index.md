...
1. Go to [Real-Time Manager](https://manager.azion.com/), access **Edge Routing **> **Intelligent DNS** menu. You will see the main interface where you can manage your zones;

2. Click on the zone you want to edit;

   You will see the main interface where you can create and edit your zones. When you do this, you will see two tabs:
...
4. To create a new record, click the **Add Record** button and fill in the following fields:
   ...
   **Name:** Identifier for the newly created Record. This field is free text. You can enter text in whatever form suits you best. A record is used to find  a computer IP address connected to the internet from a name.
   ...
   **Type:** This is the type of record that is being added, such as types A, AAAA, MX or CNAME, among other examples.<br/>[(see section - Types of supported entries and recommendations for each)](#types-of-entries).
   ...
   | **SRV - Location of server or service**     | A Service record (SRV record) is a specification of data in the Domain Name System defining the location, i.e. the hostname and port number, of servers for specified services. It is defined in RFC 2782, and its type code is 33. |
   ...
   **TTL (seconds):** This is the time a response can be cached for on a resolver server. Time-to-live (TTL) is a value in an Internet Protocol (IP) packet that tells a network router whether or not the packet has been in the network too long and should be discarded.

   **Policy:** It allows you to specify which policy iDNS should consider while resolving requests answered by this record entry.
   ...
   **Weight:** you can choose a number between and 255 to specify the weight for each record. When you choose 0, Intelligent DNS stops using this record.
