...
| $geoloc_region_name      | The remote client region name, for example “California” , ”Rio de Janeiro”. Geolocation detection by IP address. |
...
| $job_name       | Unique Azion identification for naming the type of streaming created (Ex: Data Streaming HTTP, Data Streaming WAF) |
...
| $url            | The URL where the sink was made / The URL where we sill send/sink the client data. |
...
| $client_id       | Unique Azion customer identifier.                            |
| $contentdownload | Time used to download the request content.                   |
...
| $networkduration | It returns the average volume of data received (Mb/s)        |
...
| $rendertime      | Time the browser was rendered after browsing. / The amount of time it took to render the page. |
...
| $type            | It returns the type of navigation without redirection.Indicates how the navigation to this page/script was done (Ex. navigation, reload). |
...
If you intend to search for a more generic value, you can use the “*like*” operator instead of “*equals*”, exactly like you use in SQL queries. The final query for this search can be host like `‘%[mydomain](http://mydomain.com/)%’.
