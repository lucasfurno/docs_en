...
You can transfer part of the logic of your application's business rules to Azion's Edge Node and expand these functionalities with the Application Acceleration product. That way, you build an architecture that delivers performance to your users while consuming less resources and processing at your source.
...
There are two processing phases and they represent the steps the request is in, see below: 
...
You can use as conditions of the Request Phase rules any variable related to the data sent by your user in the request. As the response has not been processed yet by your application, at this phase you don't have access to variables related to the content that will be delivered to your user.

In this phase, you also define how Azion should manage your application's cache. If your application does not allow any type of cache, you can set the conditions to ignore the cache.
...
In **Behavior** you add the behaviors you want to run, if the rule's conditions are met.
