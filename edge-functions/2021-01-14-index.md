...
*(For example, if in a particular Edge Application you determine ``true`` in the args, the function will generate a post for an s3 so the results of the request are kept.)* 
...
Check out this page for an example on using args.
